...
- 센트로이드 초기화 방법
    ...
    - k-평균++ 알고리즘
        ...
        2. $D(x^{(i)})^2/\sum_{j=1}^mD(x^{(j)})^2$의 확률로 샘플 $x^{(i)}$를 새로운 센트로이드 ($c^{(i)}$)로 선택 
...
- 최적의 클러스터 개수 찾기
    - 이너셔(클러스터 중심과 클러스터에 속한 샘플 사이의 거리의 제곱의 합)는 일반적으로 k가 증가함에 따라 점점 작아지므로 k를 선택할 때 좋은 성능 지표가 아님
...
- 단점
    ...
    - 일부 클러스터 주변에 저밀도 영역이 충분히 없는 경우 알고리즘이 잘 작동하지 않음
...
- BIC(Bayesian Information Criterion)와 AIC(Akaike Information Criterion)
    ...
    $\hat i$ : 모델의 가능도 함수(likelihood function)의 최댓값
...
- Fast-MCD
    - 이상치 탐지에 유용하고 데이터셋을 정제할 때 사용
    - 보통 샘플이 하나의 가우스 분포에서 생성되었다고 가정
        
        → 이 가우스 분포에서 생성되지 않은 이상치로 이 데이터셋이 오염되었다고 가정
        
    - 알고리즘이 가우스 분포의 파라미터를 추정할 때 이상치로 의심되는 샘플을 무시
    
    → 알고리즘이 타원형을 잘 추정하고 이상치를 잘 구분하도록 도움
    
- 아이솔레이션 포레스트
    
    고차원 데이터셋에서 이상치 탐지에 효율적
    
    - 랜덤으로 성장한 결정 트리로 구성된 랜덤 포레스트를 생성
        
        각 노드에서 특성을 랜덤하게 선택한 다음 랜덤한 임곗값을 골라 데이터셋을 나눔
        
        모든 샘플이 다른 샘플과 격리될 때까지 진행
        
    - 이상치는 일반적으로 다른 샘플과 멀리 떨어져 있으므로 평균적으로 정상 샘플과 적은 단계에서 격리됨
- LOF (Local Outlier Facotor)
    
    이상치 탐지에 좋음
    
    - 주어진 샘플 주위의 밀도와 이웃 주위의 밀도를 비교
        
        종종 k개의 최근접 이웃보다 더 격리됨
        
- one-class SVM
    
    특이치 탐지에 좋음
    
    - 하나의 클래스로 이루어진 데이터에 대해서 정상 데이터와 이상치를 나누는 평면을 찾는 알고리즘
        
        원본 공간으로부터 고차원 공간에 있는 샘플을 분리
        
        원본 공간에서는 모든 샘플을 둘러싼 작은 영역을 찾는 것에 해당
        
        새로운 샘플이 이 영역안에 놓이지 않을 경우 이상치로 판단
        
    - 매우 적은 하이퍼파라미터를 가짐
        - 커널 SVM을 위한 하이퍼파라미터
        - 마진 하이퍼파라미터: 실제 정상인 새로운 샘플을 실수로 이상치로 판단할 확률
    - 고차원 데이터셋에서 잘 작동하지만 대규모 데이터셋으로의 확장은 어려움
- PCA
...
1. 군집이란 비슷한 샘플을 구별해 하나의 클러스터 또는 비슷한 샘플의 그룹으로 할당하는 작업
    
    유사도의 개념은 주어진 문제에 따라 다름
    
2. 군집 알고리즘은 보통 고객 분류, 데이터 분석, 차원 축소 기법, 특성 공학, 이상치 탐지, 준지도 학습, 검색 엔진, 이미지 분할에 활용
3. 클러스터 개수를 선택하는 가장 간단한 방법은 엘보 규칙. 클러스터 개수의 함수로 이너셔를 그리고 그래프에서 엘보라는 지점을 찾음 → 이 지점이 최적의 클러스터 개수
    
    클러스터 개수의 함수로 실루엣 점수의 그래프를 그려 그래프에서 뾰족하게 올라간 지점의 근처가 최적의 클러스터 개수
    
4. 레이블 전파란 할당한 레이블을 동일한 클러스터에 속하는 모든 데이터에 할당하는 방법
    
    레이블을 부여하려면 비용과 시간이 많이 들고 보통 레이블이 없는 샘플이 많고 레이블이 있는 샘플은 적음 
    
5. 대규모 데이터셋으로 확장할 수 있는 알고리즘은 BIRCH와 k-편균 알고리즘
    
    밀도가 높은 지역을 찾는 군집 알고리즘은 DBSCAN과 평균-이동 알고리즘
    
6. 능동 학습은 레이블이 없는 샘플이 많고 레이블을 부여하는 데 비용이 많이 들 때 유용하고 랜덤으로 샘플을 선택해 레이블을 부여하는 것보다 능동학습이 더 바람직
    
    능동 학습은 지금까지 수집한 레이블된 샘플에 대해 학습을 진행하고 레이블되지 않은 샘플들에 대한 예측을 만듦. 이 예측에 대해 가장 낮은 예측 확률을 가진 샘플을 전문가에게 보내 레이블을 만들고 다시 학습
    
    널리 사용되는 방법인 불확실성 샘플링
    
7. 이상치 탐지는 이상치가 포함될 수 있는 데이터셋에서 알고리즘을 훈련하고 이상치 탐지의 목표는 전형적으로 (훈련 세트 안에 있는) 이상치와 새로운 샘플 사이에 있는 이상치를 구별해내는 것
    
    특이치 탐지는 깨끗하다고 가정한 데이터셋에서 알고리즘을 훈련하고 특이치 탐지의 목표는 새로운 샘플 사이에서 특이한 것을 감지하는 것
    
    아이솔레이션 포레스트는 이상치 탐지에 최적인 반면 one-class SVM은 특이치 탐지에 최적
    
8. 가우스 혼합은 샘플이 파라미터가 알려지지 않은 여러개의 가우스 분포의 합으로  생성되었다고 가정하는 모델
    
    데이터가 유한한 개수의 타원 모양 클러스터로 그룹지어 있다고 가정 
    
    하지만 샘플이 어떤 클러스터에 속해 있는지 알지 못하고 밀집도 추정, 군집, 이상치 탐지에 이 모델 사용 가능
    
9. 베이즈 가우스 혼합 모델 알고리즘을 사용하면 알고리즘이 알아서 필요하지 않은 가우스 분포의 가중치를 0으로 만듦
    
    BIC나 AIC의 그래프를 그려 BIC나 AIC를 최소화하는 클러스터 개수를 선택