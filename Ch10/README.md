...
- 전처리 하이퍼파라미터, 배치 크기와 같은 model.fit() 매개변수를 미세 튜닝해야 할 경우
    - kt.HypreModel 클래스의 서브 클래스를 만들고 build()와 fit() 메서드 두 개를 정의
        - build(): build_model() 함수와 동일한 작업을 수행해야 함
        - fit(): HyperParameters 객체와 컴파일된 모델, model.fit()의 모든 매개변수를 인수로 받아 모델을 훈련하고 History 객체를 반환
            
            하이퍼파라미터를 사용해 데이터 전처리 방법과 배치 크기 등을 결정 할 수 있음
            
    - 이 클래스의 객체를 원하는 튜너에 전달할 수 있음
    - Hyperband 튜너
        
        리소스를 할당하는 방식에서 순수 랜덤 검색보다 더 똑똑하지만 하이퍼파라미터 공간으로 랜덤으로 탐색하는 방식을 사용 
        
        → 빠르지만 덜 꼼꼼함
        
    - BayesianOptimization 튜너
        
        가우스 과정이라는 확률 모델을 적용해 하이퍼파라미터 공간의 어느 영역이 가자 유망한지 점진적으로 학습
        
        - 최적의 하이퍼파라미터를 점진적으로 찾아갈 수 있음
        - 알고리즘 자체에 하이퍼파라미터가 존재
            - alpha: 여러 번의 시도에 걸친 성능 측정에서 예상되는 잡음 수준 (default = 10^-4)
            - beta: 알고리즘이 하이퍼파라미터 공간을 얼마나 탐색할지 지정 (default = 2.6)

### 은닉 층 개수

심층 신경망은 복잡한 함수를 모델링하는 데 얕은 신경망보다 훨씬 적은 수의 뉴런을 사용하므로 동일한 양의 훈련 데이터에서 더 높은 성능을 낼 수 있음 ( = 파라미터 효율성이 좋음)

- 데이터는 계층 구조를 가지므로 심층 신경망이 유리
    
    아래쪽 은닉 층은 저수준의 구조를 모델링하고, 중간 은닉 층은 저수준의 구조를 연결해 중간 수준의 구조를 모델링
    
    가장 위쪽 은닉 층과 출력 층은 중간 수준의 구조를 연결해 고수준의 구조를 모델링
    
- 전이 학습(transfer learning): 저수준 구조를 학습할 필요 없이 새로운 신경망에서 첫 번째 신경망의 층에 있는 가중치와 편향 값으로 초기화해 고수준 구조만 학습하는 방법
...
- 은닉층
    - 저수준의 많은 특성이 고수준의 적은 특성으로 합쳐질 수 있음
        
        일반적으로 각 층의 뉴런을 점점 줄여서 깔때기처럼 구성
        
    - 대부분의 경우 모든 은닉 층에 같은 크기를 사용해도 동일하거나 더 나은 성능을 냄
        
        튜닝할 하이퍼파라미터가 층마다 한개씩이 아니라 전체를 통틀어 한개
        
    - 다른 은닉 층보다 첫번째 은닉 층을 크게 하는 것이 도움이 됨
- 네트워크가 과대적합이 시작되기 전까지 점진적으로 뉴런 수를 늘릴 수 있음
    - 필요한 것보다 더 많은 층과 뉴런을 가진 모델을 선택한 다음, 과대적합되지 않도록 조기 조료나 규제 기법을 사용
    
    → 모델에서 문제를 일으키는 병목 층(bottleneck layer)를 피할 수 있음
...
- 학습률
    ...
    - 좋은 학습률을 찾는 방법
        - 매우 낮은 학습률에서 시작해 점진적으로 매우 큰 학습률까지 반복해 무델을 훈련
