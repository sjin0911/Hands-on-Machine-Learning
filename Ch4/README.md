...
- 선형 회귀를 사용하기위한 MSE 비용 함수는 곡선에서 어떤 두 점을 선택해 그어도 곡선을 가로지르지 않는 볼록 함수 형태
...
1. 수백만 개의 특성이 있는 훈련 세트는 확률적 경사 하강법이나 미니 배치 경사 하강법을 사용 가능
훈련 세트가 메모리 크기에 맞을 경우 배치 경사 하강법도 사용 가능
    
    반면 정규방정식이나 SVD같은 경우에는 계산 복잡도가 특성 개수에 따라 매우 빠르게 증가하기 때문에 사용 불가능
    
2. 경사 하강법, 경사 하강법은 특성들이 서로 다른 스케일을 가질 경우 비용 함수는 길쭉한 타원 모양의 그래프가 되고 알고리즘이 수렴하는 데 오랜 시간이 걸림. 모델을 훈련하기 전에 데이터 스케일을 조절해야하지만 정규 방정식이나 SVD 방법은 스케일 조정 없이도 잘 작동
규제가 있는 모델은 특성의 스케일이 다르면 지역 최적점에 수렴할 가능성이 있음
규제는 가중치가 커지지 못하게 제약을 가하므로 특성값이 작으면 큰 값을 가진 특성에 비해 무시되는 경향이 있음
3. 로지스틱 회귀 모델은 모드가 하나인 그래프이므로 지역 최솟값이 없이 전역 최솟값만 존재
4. 지역 최솟값을 탈출할 수 없는 경사 하강법 알고리즘의 경우 그 값으로 수렴하지만 대부분 전역 최솟값에 수렴
    
    최적화할 함수가 볼록 함수이고 학습률이 너무 크지 않다면 모든 경사 하강법 알고리즘이 전역 최솟값에 도달하고 비슷한 결과를 만들지만 학습률을 점진적으로 감소시키지 않을 경우 SGD와 미니 배치 GD는 결국 전역 최적점 주변을 맴돌게 됨
    
5. 훈련 데이터에 과대적합 되는 중이므로 규제를 완화시킬 필요가 있음
    
    검증 오차가 지속적으로 상승한다면 학습률이 너무 높고 알고리즘이 발산할 확률이 크므로 학습률을 낮추고 훈련 오차는 증가하지 않는다면 모델이 훈련 세트에 과대적합되어있으므로 훈련을 멈추어야 함
    
6. 가장 검증 오차가 작은 하이퍼파라미터를 저장하고 중단하지 않고 더 좋은 하이퍼파라미터를 찾을 때까지 진행하고 가장 좋은 파라미터로 복원
    
    무작위성 떄문에 SGD나 미니 배치 GD는 매 반복마다 학습의 진전을 보장하지 않음
    
7. 확률적 경사 하강법은 한 번에 하나의 훈련 샘플만 사용해 훈련 반복이 가장 빨라 가장 먼저 전역 최적점 근처에 도달 (작은 미니 배치를 가진 미니 배치 GD)
    
    결과를 가장 잘 도출하는 알고리즘은 매 반복마다 모든 그레이디언트를 계산하는 배치 경사 하강법
    
    SGD와 미니 배치 GD는 학습률을 점점 낮추지 못할 경우 최적점 주변을 맴돔
    
8. 과대 적합이 발생하고 있는 상태
    
    해결을 위해 (1)다항 차수를 낮추고 (2)릿지나 라쏘를 이용해 모델을 규제하고 (3)훈련 세트의 크기를 증가 시킴
    
9. 과소 적합 이 발생하고 있는 것이고 높은 편향이 문제일 확률이 커 자유도를 높여줘야 하기 떄문에 규제 파라미터 감소 필요
10. 
    1. 규제를 가해 가중치가 매우 커지지 못하게 해 결국 과대적합을 방지
    2. 라쏘 회귀는 가중치를 완전히 0으로 만드는 경향이 있기 때문에 중요한 특성을 제외하고는 모두 0으로 만들어 특성 선택의 효과를 가져옴
    3. 라쏘의 경우 특성이 많거나 강한 연결성을 가질 경우 불규칙하게 행동할 수 있지만 엘라스틱넷이 라쏘 회귀를 이용해 불규칙성을 감소 시킴
11. 두 개의 로지스틱 회귀 분류기 필요, 이 둘은 배타적인 클래스가 아니고 네 가지 조합이 모두 가능